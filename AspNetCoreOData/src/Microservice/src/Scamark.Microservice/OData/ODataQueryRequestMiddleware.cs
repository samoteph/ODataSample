...
public class ODataQueryRequestMiddleware
{
    ...
    internal static async Task TransformQueryRequestAsync(IODataQueryRequestParser parser, HttpRequest request)
    {
        ...
        {
            if (IsODataFunction(queryOptions))
            {
                requestPath += queryOptions;
            }
    ...
    private static bool IsODataFunction(string queryOptions)
        => queryOptions.StartsWith('(') && queryOptions.EndsWith(')');
}
